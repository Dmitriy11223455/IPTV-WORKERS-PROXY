// СКРИПТ 2: ПРОКСИ-ВОРКЕР
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);

    // 1. РОУТ: ГЕНЕРАЦИЯ ТОКЕНОВ ДЛЯ БОТА
    if (request.method === 'POST' && url.pathname === '/generate') {
      const secretKey = request.headers.get('X-Secret-Key');
      if (secretKey !== env.SECRET_INTER_KEY) {
        return new Response('Unauthorized', { status: 401 });
      }

      // Генерация 12-значного токена
      const proxyToken = crypto.randomUUID().replace(/-/g, "").substring(0, 12);
      
      return new Response(JSON.stringify({ token: proxyToken }), {
        headers: { 'Content-Type': 'application/json' }
      });
    }

    // 2. РОУТ: ОБРАБОТКА ЗАПРОСОВ ОТ IPTV ПЛЕЕРА
    if (request.method === 'GET' && url.pathname === '/proxy') {
      const token = url.searchParams.get('token');
      const targetUrl = url.searchParams.get('url'); // searchParams.get автоматически декодирует URL

      if (!token || !targetUrl) {
        return new Response('Missing parameters', { status: 400 });
      }

      // Проверка токена в БД на GitHub (только чтение)
      const ghUrl = `https://api.github.com/repos/${env.GH_OWNER}/${env.GH_REPO}/contents/${env.GH_PATH}`;
      const ghHeaders = {
      ...
      try {
        const ghReq = await fetch(ghUrl, { headers: ghHeaders });
        if (!ghReq.ok) {
          return new Response('Database not found or unreachable', { status: 500 });
        }
        ...
        // Поиск токена
        const tokenIsValid = db.some(entry => entry.proxy_token === token);
        if (!tokenIsValid) {
          return new Response('Access Denied: Invalid or expired token', { status: 403 });
        }
...
async function handleRequest(request) {
  ...
  try {
    ...
    return new Response('Worker Error: ' + e.message, { status: 500, headers: corsHeaders });
        ...
        // Создаем новые заголовки для обхода, пробрасывая заголовки оригинального плеера
        const proxyHeaders = new Headers(request.headers);
        // Очищаем Host, чтобы fetch подставил Host целевого сервера
        proxyHeaders.delete('Host'); 
        // Дополнительные модификации заголовков (User-Agent, Referer) при необходимости геоблока...
        ...
        // Возвращаем поток плеера обратно
